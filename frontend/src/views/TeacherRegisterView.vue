<template>
  <div class="auth-page">
    <div class="auth-card">
      <h1 class="auth-title">教师注册</h1>
      <p class="auth-subtitle">创建教师账号，管理您的课程</p>
...
async function handleRegister() {
  ...
  loading.value = true
  try {
    await authStore.registerTeacher(form.username, form.password, form.name)
    successMsg.value = '教师注册成功！即将跳转到登录页...'
    setTimeout(() => {
      router.push('/login')
    }, 1500)
  } catch (e) {
    errorMsg.value = e.displayMessage || e.message || '注册失败，请稍后重试'
  } finally {
    loading.value = false
  }
}
