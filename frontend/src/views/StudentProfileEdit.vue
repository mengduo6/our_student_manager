<template>
  <div class="page-container">
    <div class="page-card">
      ...
      <p class="page-desc">修改您的个人基本信息。部分字段（如用户名、班级）不可直接修改。</p>
      ...
      <form class="form" @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          <label class="form-label">所在班级 <span class="label-tag">不可修改</span></label>
          <input type="text" class="form-input readonly" :value="form.className || '未分配'" disabled />
        </div>
        ...
        <div class="form-actions">
          ...
          <router-link to="/student" class="btn btn-outline">取消</router-link>
        </div>
...
onMounted(async () => {
  try {
    ...
    errorMsg.value = '加载个人信息失败: ' + (e.displayMessage || e.message || '网络错误')
  }
})
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.major !== originalForm.value.major) payload.major = form.major ? form.major.trim() : ''
